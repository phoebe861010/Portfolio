...
spans.forEach(span=> span.addEventListener('mouseout',function(e){span.ClassList.remove('animated','rubberBand')}))
...
t1.fromTo(htmlBar, .75,{width:`calc(0%-6px)`},{width:`calc(85%-6px)`,ease: Power4.easeOut} )
...
const showRequiredCategory = event =>{
  ...
  for(i=0; i<categories.length; i++){
    if(categories[i].hasAttribute('class')){
      categories[i].classList.remove('showCategory') categories[i].classList.add('hideCategory')
    }
  }
